...
대입 연산자는 변수에 값을 저장하기 위한 연산자이다. 가장 기본적인 형태의 대입 연산자는 다음과 같이 오른쪽 피연산자의 값을 왼쪽 피연산자인 변수에 저장하는 것이다.
...
이렇게 기존의 변수 값을 기준으로 특정 값만큼 더하거나, 빼거나, 곱하거나, 나누는 등의 연산을 수행하여 변수의 값을 수정하는 연산자를 복합 대입 연산자라고 한다. 더하기, 빼기, 곱하기, 나누기 기호가 대입 연산 기호의 왼쪽에 오는 것에 주의해야 한다. 더하기나 빼기 기호가 우측에 위치하면 아예 다른 의미가 된다.  
...
참고로 간혹가다가 ```a = a + b```와 ```a += b```가 완전히 같다고 이야기하는 자료도 있지만 절대 그렇지는 않다. C언어를 처음 공부한다면 지금은 이해할 수 없겠지만 ```a[i++] = a[i++] + b```와 ```a[i++] += b```는 엄연히 다르다. 이를 지금 이해할 필요는 없지만, 저 둘이 완전히 동일한 것은 아니다라는 것만 확인하고 넘어가도록 하자.  
