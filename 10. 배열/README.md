...
하지만 가변 길이 배열의 문제는 다른 곳에 있다. 우선 C99부터 표준에 도입되기는 하였지만, 여러가지 논란으로 인해 C11 표준 이후부터는 선택 구현으로 변경되었다. 따라서 특정 환경에서는 가변 길이 배열을 지원하지 않을 수도 있다. (예를 들어 윈도우의 msvc를 사용하면 가변 길이 배열을 지원하지 않는다.)  
