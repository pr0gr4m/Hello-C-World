...
배열의 요소를 위한 타입에는 ```void```를 포함한 불완전(incomplete) 타입과 함수 타입은 올 수 없다. 그를 제외한 모든 타입이 올 수 있으며, 배열의 길이를 위한 *constant-expression*는 결과가 0을 초과하는 정수여야 한다.  
...
int main(void)
{
    ...
    for (int i = 0; i < INPUT_SIZE / 2; i++) {
        temp = arr[i];
        arr[i] = arr[INPUT_SIZE - i - 1];
        arr[INPUT_SIZE - i - 1] = temp;
    }
