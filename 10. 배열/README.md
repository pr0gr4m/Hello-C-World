...
> 배열의 배열과 다차원 배열을 구분하는 언어도 존재하지만, C에서는 두 개념을 동일하게 취급한다. 구분하는 언어의 경우 보통 ```arr[i, j]``` 와 같은 다차원 인덱싱 문법을 지원하는데, C에서는 이러한 문법은 지원하지 않는다. C에서는 다차원 배열 인덱싱을 ```arr[i][j]```와 같이 작성해야 한다.  
