...
하지만 ```i < sizeof(arr) / sizeof(arr[0])```는 컴파일러와 옵션에 따라 오류를 발생시킬 수도 있다. ```sizeof``` 연산의 결과는 ```size_t``` 라는 타입을 갖는다. 해당 타입은 크기를 표현하기 위해 사용하는 부호 없는 정수 타입이다. 보통 ```unsigned int``` 타입이나 ```unsigned long```과 같은 타입을 재정의한 타입이다. 우선 ```unsigned long``` 타입이라고 가정해보자.  
