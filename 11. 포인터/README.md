...
동일한 타입의 포인터끼리는 대입 연산이 가능하다. 예를 들어 다음과 같다.  
...
![assign](https://github.com/pr0gr4m/Hello-C-World/blob/main/img/%ED%8F%AC%EC%9D%B8%ED%84%B0/assign.png?raw=true)  

p1와 p2 모두 동일한 객체 a를 가리키고 있다. 따라서 p1에 간접 접근하여 값을 수정해도 a가 변경되며, p2에 간접 접근하여 값을 수정해도 a가 변경된다. ```a```, ```*p1```, ```*p2``` 는 모두 같은 값을 읽게 된다.  

이렇게 포인터 간의 대입 혹은 초기화가 가능하며, 같은 객체를 여러 포인터가 가리킬 수 있다는 것을 간단히 살펴봤다. 그런데 완벽히 같은 타입의 포인터끼리만 대입 혹은 초기화가 가능할까? 조금 더 유연한 규칙이 존재한다.  
우선 서로 호환되는(compatible) 포인터 타입끼리는 대입 혹은 초기화가 가능하다. 참고로 타입의 호환(compatible) 규칙에 대해서는 추후에 다시 알아볼 것인데, 우선 두 타입이 동일하면 서로 호환된다고 생각하자. (서로 표현 방식이 동일한 타입은 호환 가능하다고 생각하면 된다.)  
그리고 두 포인터 타입이 완전히 호환되지 않을 때의 규칙은 다음과 같다.  
왼쪽 피연산자와 오른쪽 피연산자의 타입 지정자가 동일할 때(엄밀히 이야기하면 호환될 때), 왼쪽 피연산자가 오른쪽 피연산자의 타입 한정자를 모두 포함하면 된다. 여태까지 배운 타입 한정자로는 ```const```가 있는데, 예시를 위해 ```volatile``` 이라는 한정자를 추가로 사용할 것이다. ```volatile``` 한정자가 무엇인지는 추후에 학습할 예정이니, 한정자라는 사실만 알고 있자.  
