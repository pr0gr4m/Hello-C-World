...
재귀(recursion)는 정의할 때 자기 자신을 재참조하는 방법을 뜻한다. 대부분 재귀 호출(recursive call)의 형태로 구현하며, 재귀 호출하도록 정의한 함수를 보통 재귀 함수(recursive function)라고 한다.  
설명은 어렵지만 예시를 보면 간단하게 이해할 수 있다. 다음은 C언어로 정의할 수 있는 가장 간단한 재귀 함수이다.  
```c
void recursive()    // recursive 함수 정의
{
    recursive();    // recursive 함수 호출
}
```  

recursive 라는 함수를 정의하는데, 그 몸체에서 다시 자기 자신인 recursive 함수를 호출하였다. 이런게 어떻게 가능할까? 그리고 이러한 함수는 어디에 쓸모가 있을까?  

사실 재귀 함수의 기원은 논리학, 언어학, 수학과 같은 다른 학문에서 유래되었다. 따라서 저러한 분야들의 문제들을 컴퓨터를 이용하여 해결하려는 경우 극강의 표현력을 제공해준다. 예를 들어 수학의 점화식(recurrence relation)은 재귀식이라고도 부르는데, 이 점화식(재귀식)을 재귀 함수로 표현하여 N항을 구할 수 있다. 이 외에도 어떠한 거대한 문제를 풀기 쉬운 부분 문제로 나눠서 해결하는 상황 등에서 유용하게 사용할 수 있다.  

재귀함수로 풀 수 있는 대표적인 문제로 피보나치의 N항을 구하는 예제를 살펴보자. 피보나치 수는 첫째 및 둘째 항이 1이며 그 뒤의 모든 항은 바로 앞 두 항의 합인 수열이다. 점화식으로 표현하면 다음과 같다.  

$F_{1} = F_{2} = 1$  
$F_{n} = F_{n - 1} + F{n - 2}$  

다음은 피보나치 수의 N항을 재귀 함수로 계산하는 예제이다.  
...
{
    int n;
    printf("1 이상의 N을 입력하세요 : ");
    scanf("%d", &n);

    if (n <= 0) {
        // 예외 처리
        printf("%d이 0보다 작습니다.", n);
        return -1;
    }
...
int fibonacci(int n)
{
    if (n == 1 || n == 2)       // 1항과 2항의 경우
        return 1;
    return fibonacci(n - 1) + fibonacci(n - 2);     // 재귀 정의
}
```  
...
```bash
pr0gr4m@DESKTOP-IRB9MN5:~/src$ make fibonacci
cc     fibonacci.c   -o fibonacci
pr0gr4m@DESKTOP-IRB9MN5:~/src$ ./fibonacci 
1 이상의 N을 입력하세요 : 1
피보나치 1항 : 1
pr0gr4m@DESKTOP-IRB9MN5:~/src$ ./fibonacci 
1 이상의 N을 입력하세요 : 2
...
입력한 N에 대한 피보나치 항을 구하는걸 볼 수 있다. 재귀 함수를 처음 접하면 어떻게 이런 결과가 나오는지 조금 생소할 수 있다. 다음 그림은 피보나치 5항을 재귀 함수로 구할 때 함수 호출 단계들을 보여준다.  
![fibo](https://github.com/pr0gr4m/Hello-C-World/blob/main/img/%ED%95%A8%EC%88%98/fibo.png?raw=true)  

```fibonacci(2)```와 ```fibonacci(1)```은 각각 1을 반환한다. ```fibonacci(3)```은 ```fibonacci(2) + fibonacci(1)``` 라는 식을 수행하여 1 + 1인 2를 반환한다. ```fibonacci(4)```은 ```fibonacci(3) + fibonacci(2)``` 라는 식을 수행한다. 3항은 이 전과 같이 1 + 1인 2를 반환하고, 2항은 1을 반환하여, 4항은 결국 2 + 1인 3을 반환한다. 이런 식으로 수열의 각 항들을 재귀적으로 구해나간다.  
이해가 될듯말듯 아리송할 것이다. 전에 반복문에 적응하기 위하여 여러 연습 문제들을 풀어봤던 것처럼, 재귀 함수도 다양한 연습 문제를 풀면서 각 함수의 호출 단계들을 그림으로 그려보면 익숙해질 것이다.  

그런데 위 피보나치 예제에 문제가 있다. 30항까지는 잘 구해진다. 40항부터는 조금 버벅이지만 구하는데 문제 없다. 50항부터는 도대체 언제 결과가 나오나 싶으며, 항이 너무 커지면 프로그램이 실행 도중 오류로 강제 종료한다.  
왜 이런 문제가 발생할까? 크게 보면 두 가지 문제가 있다. 우선 항이 늘어날수록 너무 많은 함수가 호출된다. 그림에서 보는 것처럼 5항에서는 함수를 9번 호출한다. 이 호출 횟수가 기하급수적으로 증가하여 10항에는 109번, 20항에는 13529번, 30항은 1664079번, 40항은 204668309번의 함수가 호출된다.  
그리고 함수의 동작 원리에서 살펴봤듯이, 호출된 함수는 반환하기 전까지 스택 영역에 스택 프레임을 차지한다. 실제 스택 프레임은 복귀 주소와 지역 변수 외에도 다양한 정보를 저장하고 있기 때문에 메모리를 적지 않게 사용한다. 그런데 재귀 함수는 가장 깊은 자식 함수를 호출하기 전까지 그 부모 함수들이 반환하지 않기 때문에 많은 스택 프레임이 메모리에 상주하게 된다. 이러다가 스택 메모리 영역을 모두 사용해버리면 스택 오버플로우(stack overflow)라는 오류가 발생하여 프로그램이 강제 종료하는 것이다. 참고로 리눅스에서 스택 사이즈는 다음과 같은 명령어로 확인할 수 있다.  
```bash
pr0gr4m@DESKTOP-IRB9MN5:~/src$ ulimit -a | grep stack
stack size              (kbytes, -s) 8192
```  
기본적으로 8192KB(8MB)로 설정되어 있다. 따라서 별도의 설정을 하지 않으면 스택 프레임이 쌓이다가 8MB가 넘어가면 에러가 발생할 것이다.  

이러한 문제들을 해결하기 위한 다양한 방법이 있다. 기하급수적인 함수 호출 횟수 증가는 보통 알고리즘적으로 최적화하는데, 해당 서적의 범위를 벗어난다. 이에 대한 내용이 궁금하다면 쉽게 배우는 알고리즘(문병로 저)이나 Introduction to Algorithms(CLRS 저)와 같은 알고리즘 서적들의 다이나믹 프로그래밍과 같은 최적화 파트를 참고하면 된다.  
그렇다면 스택 오버플로우 문제도 알고리즘적으로 해결해야 할까? 물론 잘 최적화하여 함수 호출 깊이를 줄일 수 있다면 어느 정도 해결할 수 있겠지만, 이는 재귀 함수의 고질적인 문제다. 따라서 일반적으로 최대한 재귀 함수 사용을 지양하는 방향으로 해결한다.  
사실 거의 모든 재귀 함수는 반복문이나 goto문을 이용하여 같은 결과를 만들어내도록 표현할 수 있다. 예를 들어 위 피보나치 예제를 다음과 같이 반복문이나 goto 문으로 변경할 수도 있다. (이를 변경하는 내용은 마찬가지로 해당 서적의 범위를 벗어나므로, 예제 소스 코드가 완벽히 이해가지 않아도 괜찮다.)  
...
int main(void)
{
    ...
    printf("피보나치 %d항 : %d(iter), %d(goto)\n", n, fibonacci_iter(n), fibonacci_goto(n));
    return 0;
}
...
실행해보면 이 전 예제와 같은 결과를 볼 수 있다. 단지 이 전과 다르게 40항을 구할 때도 50항을 구할 때도 즉각 반응한다. (그런데 현재 환경을 기준으로 47항부터 갑자기 이상한 결과가 발생한다. 왜 그런지 생각해보자. 힌트는 int 값의 범위이다.)  
결과만 놓고 보면 재귀 함수는 단점 뿐이고 사용하면 안 되는 것만 같다. 상황에 따라 그럴 수도 있다. 상용 서비스에서 크기가 정해지지 않은 데이터를 재귀 함수로 처리하려는 행위는 절대로 하면 안 된다. 꼬리 재귀 최적화라는 기법을 통하여 문제를 어느 정도 상쇄할 수는 있지만 되도록 이러한 상용 서비스에서는 재귀의 사용을 지양하는 것이 좋다.  
하지만 데이터 크기의 제한이 정해져 있는 알고리즘적 문제 해결 분야라던지, 적은 수의 데이터 크기가 고정적인 서비스 분야라면 경우에 따라 사용하더라도 큰 문제가 되지 않는다. 위에는 단점을 위주로 이야기 했지만, 재귀 함수를 사용한 피보나치 예제와 사용하지 않은 예제 소스 코드를 비교하면 사용한 경우가 훨씬 간단하다. 재귀 함수가 문제의 소지가 되더라도 이렇게 극강의 표현력을 제공한다. 따라서 그 성질을 정확하게 파악하여 문제 소지가 전혀 발생할 경우가 없을 때만 제한적으로 사용한다면 분명 이점도 있다. 그런데 일반적인 상용 서비스의 경우 문제 소지라는 것이 현재 상황에서의 문제만이 아닌, 미래 상황의 문제도 고려해야 하기 때문에 되도록 사용을 지양하는 편이다.  

재귀 함수의 사용처, 사용법, 장단점에 대해 간단하게 알아봤다. 재귀 함수의 동작 방식을 좀 더 자세히 이해할 수 있도록 예제를 하나 살펴보고, 재귀 함수의 동작 원리에 대해 알아보자.  
다음 예제는 입력받은 십진수를 재귀함수를 이용하여 이진수의 형태로 출력한다.  

```c
// decimal_to_binary.c
#include <stdio.h>

void print_to_binary(int n);

int main(void)
{
    int num;
    printf("0 이상의 정수(십진수)를 입력하세요 : ");
    scanf("%d", &num);
    printf("이진수 : ");
    print_to_binary(num);
    printf("\n");
...
```bash
pr0gr4m@DESKTOP-IRB9MN5:~/src$ make decimal_to_binary
cc     decimal_to_binary.c   -o decimal_to_binary
pr0gr4m@DESKTOP-IRB9MN5:~/src$ ./decimal_to_binary 
0 이상의 정수(십진수)를 입력하세요 : 10
이진수 : 1010
```  

이 전 예제보다 조금 난해할 수 있으니 ```print_to_binary``` 함수를 라인별로 살펴보자.  
* ```if (n / 2)``` : n을 2로 나눈 결과가 0이 아니면 참이다. n이 0이나 1이면 거짓이고, 2 이상이면 참일 것이다.  
* ```print_to_binary(n / 2);``` : 재귀 호출 부분으로, 인자에 n을 2로 나눈 결과 값을 전달하고 있다. n이 10으로 시작하면 각 n을 2로 나눈 값인 5(10 / 2) -> 2(5 / 2) -> 1(2 / 2) 순으로 전달할 것이다. n이 1 이하일 때는 위 if 문이 거짓이므로 실행되지 않는다.  
* ```printf("%d", n % 2);``` : n을 2로 나눈 나머지를 출력하고 있다. ```print_to_binary``` 함수의 마지막 호출에서 n의 값이 1이었다. n이 1인 호출에서 가장 먼저 해당 라인에 도달하여 1 % 2인 1을 출력하고 반환한다. 그러면 n의 값이 2인 호출로 돌아와서 2 % 2인 0을 출력하고 반환한다. 그리고 n의 값이 5인 호출로 돌아와 5 % 2인 1을 출력하고 반환하며, n의 값이 10인 호출(최초 호출)로 돌아와서 10 % 2인 0을 출력하고 반환한다.  

위 과정을 그림으로 표현하면 다음과 같다.  
![recursive](https://github.com/pr0gr4m/Hello-C-World/blob/main/img/%ED%95%A8%EC%88%98/recursive.png?raw=true)  

```print_to_binary``` 함수를 재귀 호출할 때, 호출자는 해당 함수 호출부에서 대기하다가 호출했던 함수가 반환하면 해당 라인의 다음 내용으로 복귀하여 실행을 이어나간다. n이 2인 경우 ```if (n / 2) print_to_binary(n / 2);``` 까지 실행하여 ```print_to_binary(1);```과 같이 호출하고 해당 라인에서 대기한다. 그러면 n이 1인 ```print_to_binary``` 함수가 실행되어 if 문은 거짓이므로 넘어가고 ```printf("%d", n % 2);```를 실행한다. 여기에서 1 % 2인 1을 출력하고 반환하면 이 전 n이 2인 호출로 돌아간다. 기존에 ```print_to_binary(1);``` 에서 대기중이었는데, 이 다음부터 실행을 이어나간다. 그러면 다음 라인인 ```printf("%d", n % 2);```를 실행하면서 2 % 2인 0을 출력하는 것이다.  

이렇게 라인별 호출과 복귀 단계를 이해하고 나면 재귀 함수의 동작 방식을 파악할 수 있을 것이다. 그런데 어떻게 이런 일이 가능한 것일까? 함수가 실행 도중에 자기 자신을 호출하는 동작에 대한 의아함이 생길 수도 있을 것이다. 이 전에 학습했던 코드 영역과 스택 영역을 복기하면 동작 원리를 이해할 수 있다.  
프로그램이 실행되어 메모리에 로드되면 코드 영역에 실행할 명령어들은 이미 모두 올라와있는 상태이다. 예를 들어 코드 영역에 각 문장이 다음과 같이 올라와있다고 가정해보자.  
```c
// statement 당 명령어 하나라고 가정, 명령어 하나당 4 바이트라고 가정
// 아래 명령어들은 모두 해당 프로세스의 코드 영역에 로드됨
void print_to_binary(int n)
{
    if (n / 2)                      // 0x1000 번지
        print_to_binary(n / 2);     // 0x1004 번지
    printf("%d", n % 2);            // 0x1008 번지
}
```  
그리고 프로그램이 실행되어 실제로 ```print_to_binary``` 함수를 실행하면 스택 영역에 스택 프레임이 할당된다. (main 함수를 제외하고) 처음 할당된 스택 프레임에는 main 함수로의 복귀 주소와 파라미터 변수 n이 포함되어 있을 것이며, 해당 n의 값은 10이다. 0x1000 번지의 명령어를 CPU로 가져와서 실행하려고 하는데, n의 값이 뭔지 알아야 한다. 스택 영역에서 n의 값인 10을 가져와서 명령어를 수행했더니 참으로 판단되어 다음 명령어를 수행한다. 0x1004 번지의 명령어를 CPU로 가져와서 실행하는데 이는 함수 호출이다. 새로운 스택 프레임을 할당하여 복귀 주소를 0x1008 번지로 저장하고 파라미터 변수 n의 값을 5로 저장한다. 이 후에 위와 같은 과정을 반복하면 결국 n의 값이 1인 함수의 호출까지 도달하여 해당 스택 프레임이 올라올 것이다. 그러면 이 전과 마찬가지로 0x1000 번지를 수행하지만, 이번에는 스택 프레임에서 가져온 n의 값이 1이므로 결과가 거짓이다. 따라서 0x1004 번지의 명령어를 실행하지 않고 0x1008번지의 명령어로 넘어간다. 해당 명령어의 결과로 1 % 2인 1을 출력한다. 0x1008 번지가 해당 함수의 마지막 명령어이므로 함수를 반환한다. 복귀 주소인 0x1008 번지로 돌아가면서 가장 최근에 할당되었던 스택 프레임은 해제된다. 0x1008 번지의 명령어를 수행하면서 n의 값을 스택 프레임에서 가져오는데, 이번 스택 프레임에서의 n의 값은 2이다. 따라서 2 % 2인 0을 출력한다. 마찬가지로 함수를 반환하며 복귀 주소인 0x1008 번지로 돌아가면서 스택 프레임을 해제한다. 위와 같은 동작을 반복하여 마지막 0x1008 번지 명령어 수행 시에 n의 값을 스택 프레임에서 가져오면 n이 10이므로 10 % 2의 결과인 0을 출력하고 함수를 반환한다. 그러면 재귀 함수의 모든 동작이 종료하고 main 함수로 복귀하는 것이다.  
위 과정의 콜 스택 변화 과정을 그림으로 표현하면 다음과 같다.  
![recur_stack](https://github.com/pr0gr4m/Hello-C-World/blob/main/img/%ED%95%A8%EC%88%98/recur_stack.png?raw=true)  

위의 내용들이 이해가지 않는다면 휴식 시간을 가진 후 천천히 다시 읽어보자. 어느 정도 난이도가 있기 때문에 이해가 쉽지 않을 수도 있다. 그래도 위와 같이 동작 방식과 동작 원리를 모두 이해하고 나면 코드 영역과 스택 영역의 필요성도 더욱 와닿을 것이며, 추후에 알고리즘 과목을 학습할 때 재귀로 구현하는 기법들을 훨씬 수월하게 이해할 수 있을 것이다.  
재귀 함수를 처음 학습하면 상세하게 이해하기 어려운데 정작 상용 서비스에는 사용을 지양해야 하기 때문에 동작 방식만 간단하게 소개하는 경우도 있다. 하지만 확실하게 학습하고 나면 일반적인 함수의 동작 원리까지 더욱 깊게 이해할 수 있기 때문에 해당 서적에서는 분량을 꽤나 투자했다. 이번에도 학습을 마친 독자분들에게 격려의 말씀을 남긴다.  
