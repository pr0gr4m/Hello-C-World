...
사실 이 외에도 두 엔디안 방식 중에 하나를 선택할 수 있는 바이 엔디안 방식이나, 두 엔디안을 섞어서 사용하는 미들 엔디안 방식도 존재한다. 임베디드 기기나 모바일에서 주로 사용하는 ARM 프로세서의 경우 대부분 바이 엔디안이며, 일부 소수 표현에 미들 엔디안 방식을 사용하기도 한다.  
...
exec 계열 함수는 운영체제에게 현재 프로세스를 요청한 프로그램의 정보로 교체해달라는 요청을 한다. 요청 받은 운영체제는 우선 현재 실행 중인 프로세스의 정보들을 날려버린다. 그리고 실행을 요청한 프로그램의 실행 파일을 읽어서 ELF 정보를 해석한다. 해석한 ELF 파일의 각 섹션 정보를 바탕으로 프로세스 메모리의 각 영역(세그먼트)에 필요한 데이터들을 채워넣는다. 이 후 프로그램 실행을 위한 나머지 처리들을 수행해줄 ELF 인터프리터(interpreter)를 설정하고 exec 함수는 종료된다. ELF 인터프리터란 ELF 파일을 실행할 수 있도록 해석해주는 프로그램으로, 다이나믹 링킹이나 main 함수의 실행과 같은 일들을 수행해준다. 다이나믹 링킹에 대한 이야기는 별도 링킹 챕터에서 추가로 학습할 것이다.  

이렇게 프로그램이 실행되어 메모리에 올라오는 과정을 프로그램 로드(load)라고 한다. 로드를 수행하는 프로그램을 로더(loader)라고 하는데, 현대 많은 경우 운영체제에 로더가 내재되어 있다.  

### 여러 개의 프로세스

그런데 위의 과정을 거쳐 프로그램이 로드되어 프로세스가 되었다고 해서 무조건 바로 실행되는 것은 아니다. 우리가 사용하고 있는 대부분의 현대 운영체제에는 여러 프로세스가 동시에 실행 상태로 놓여져 있다. 따라서 여러 프로세스 중 누가 CPU를 차지하여 실제로 실행할지를 결정해야 한다.  
