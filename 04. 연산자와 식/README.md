...
이번 챕터의 초반부에서 식(expression)이란 값을 계산하기 위하여 연산자, 피연산자, 오브젝트, 함수, 사이드 이펙트 발생들이 조합되는 것이라고 했다. 그리고 평가(evaluation)란 식의 값을 계산하거나 사이드 이펙트가 시작하는 것이라고 했다.  
그러면 식은 어떤 순서로 평가될까? 이는 연산자의 우선순위, 결합 방향, 하위 식의 평가 순서에 따라 결정된다.  

### 연산자 우선순위

C언어에 있는 연산자들은 다음 표와 같은 우선 순위와 결합 방향을 갖는다.  

| 순위(Precedence) | 연산자(Operator) | 설명 | 결합 방향(Associativity) |
|:-----:|:-----|:--------|:---------|
| 1 | ```(expression)``` <br> ```identifier``` <br> ```constant``` <br> ```string-literal``` <br> ```generic-selection``` | 식 괄호 <br> 식별자 <br> 상수 <br> 문자열 리터럴 <br> 제너릭 셀렉션 | |
...
위 표에서 설명 칸에 연산자라는 단어가 붙지 않은 요소들은 연산자는 아니지만 단독으로 식을 구성할 수 있는 요소들이다.  
(대표적으로 변수의) 식별자, 상수, 문자열 리터럴과 같은 것들은 단독으로 식을 구성할 수 있으며 이는 가장 높은 우선 순위를 갖는다. 그리고 모든 식은 괄호로 묶어줄 수 있으며, 괄호로 묶은 식은 앞에 있는 단일 요소들과 같이 가장 높은 우선 순위를 갖는다.  
한 식에 있는 연산자들은 일반적으로 우선 순위가 높을 수록 먼저 평가되며, 한 식에 우선 순위가 같은 연산자들이 여러 개가 사용되면 결합 방향 순서로 평가된다.  
예를 들어 다음과 같은 식이 있을 때 어떤 순서로 평가가 될 지 알아보자.  

```c
a = b == c ? d++ + e * f : g - --h - i
```

위 식에서 연산자 우선 순위는 후위 증가 연산자 > 전위 감소 연산자 > 곱하기 연산자 > 더하기 연산자 = 빼기 연산자 > 동등 연산자 > 조건 연산자 > 대입 연산자 순으로 높다. 이에 맞춰서 괄호를 통해 식이 평가될 순서를 다음과 같이 차례대로 표기할 수 있다.  

```c
a = b == c ? (d++) + e * f : g - --h - i                    // 후위 증가 연산자
a = b == c ? (d++) + e * f : g - (--h) - i                  // 전위 감소 연산자
a = b == c ? (d++) + (e * f) : g - (--h) - i                // 곱하기 연산자
a = b == c ? ((d++) + (e * f)) : g - (--h) - i              // 더하기 연산자
a = b == c ? ((d++) + (e * f)) : (g - (--h)) - i            // 빼기 연산자 (결합 방향상 왼쪽 우선)
a = b == c ? ((d++) + (e * f)) : ((g - (--h)) - i)          // 빼기 연산자
a = (b == c) ? ((d++) + (e * f)) : ((g - (--h)) - i)        // 동등 연산자
a = ((b == c) ? ((d++) + (e * f)) : ((g - (--h)) - i))      // 조건 연산자
(a = ((b == c) ? ((d++) + (e * f)) : ((g - (--h)) - i)))    // 대입 연산자
```

최종적으로 연산자 우선 순위와 결합 방향을 고려하여 괄호를 작성한 식은 가장 아래와 같다. 이렇게 모든 연산자에 대해 괄호를 작성한 식을 완전 괄호 식(fully parenthesized expression)이라고 한다. 참고로 빼기 연산자의 결합 방향은 Left-to-right 이므로 ```a - b - c```와 같은 식이 있을 때 ```((a - b) - c)```와 같이 작성할 수 있다.  
그러면 위와 같은 식에서 무조건 연산자 우선 순위대로 ```d++```가 가장 먼저 평가될까? 아쉽지만 그렇지는 않다. 이는 하위 식의 평가 순서에 따라 결정된다.  

### 하위 식 평가

식의 가장 기본적인 형태는 식별자, 상수, 문자열 리터럴과 같은 것들은 단독으로 사용하거나, 이들을 하나의 연산자의 피연산자로 사용하는 것이다. ```a```, ```3```, ```"abc"```, ```a + 3```, ```a = 3```과 같은 것들이 가장 기본적인 형태의 식이 된다.  
하지만 예시를 통해 여러 번 접했던 것과 같이 여러 개의 연산자를 조합하여 하나의 복잡한 식을 만들 수 있다. ```a * b  + c / d```와 같이 한 식에 여러 개의 연산자를 사용할 수 있다. 이 식은 ```((a * b) + (c / d))```와 같이 괄호를 사용해서 표현할 수 있다. 이 때, ```a * b```나 ```c / d```와 같이 식 내에서 독립적으로 구성할 수 있는 식을 하위 식(subexpression)이라고 한다.  
바로 위 식에서 곱하기와 나누기가 더하기보다 먼저 평가(발생)된다는 것은 자명하다. 그러면 곱하기와 나누기 중에는 무엇이 먼저 평가될까? 결합 방향이 왼쪽에서 오른쪽이므로 곱하기가 먼저 평가될까? 그렇지 않다.  
결합 방향은 식의 결합 순서를 결정하였다면 그 역할을 모두 수행한 것이다. 즉, ```a - b - c```와 같은 식을 ```((a - b) - c)```으로 해석하게 하거나, ```a = b = c```와 같은 식을 ```(a = (b = c))```으로 해석하게 만들었다면 그 역할이 종료된 것이다. 위와 같이 서로 다른 하위 식 사이의 연산 순서를 결정하지는 않는다.  
그렇다면 하위 식의 평가 순서는 어떻게 될까? 이는 미지정 행위(Unspecified Behavior)이다. 즉 ```((a * b) + (c / d))```과 같은 식에서는 ```a * b```가 먼저 평가될 수도 있고 ```c / d```가 먼저 평가될 수도 있는 것이다. 프로그래머는 이를 알 수 없다. 심지어 ```(a + b) - (a * b)```와 같은 식이 있다고 한다면 곱하기 연산자가 더 우선 순위가 높지만, 이미 두 개의 하위 식으로 나뉘어 졌으므로 어느 하위 식이 먼저 평가될지 알 수 없다. 이런 경우엔 ```a + b```가 ```a * b```보다 먼저 평가될 수도 있는 것이다.  
단, 예외가 있다. 논리 연산자(```&&```, ```||```)와 컴마 연산자(```,```)는 왼쪽 피연산자로 쓰인 하위 식이 오른쪽 피연산자로 쓰인 하위 식보다 먼저 평가된다. 그리고 조건 연산자(```? :```)는 첫 번째 피연산자로 쓰인 하위 식이 두 번째나 세 번째 피연산자로 쓰인 하위 식보다 먼저 평가된다. 이 외에도 함수 호출과 관련된 평가 순서 보장이 하나 더 있지만 이는 함수 챕터에서 다시 알아볼 것이다.  

여태까지 배운 내용을 기반으로 다음 식의 평가 순서를 다시 생각해보자.

```c
(a = ((b == c) ? ((d++) + (e * f)) : ((g - (--h)) - i)))
```

우선 괄호상 ```((b == c) ? ((d++) + (e * f)) : ((g - (--h)) - i))``` 라는 하위식이 먼저 평가된다. 해당 하위 식에서는 조건 연산자가 사용되었으므로 첫 번째 피연산자의 하위 식이 가장 먼저 평가될 것이 보장된다. 따라서 ```(b == c)```가 가장 먼저 평가된다.  
```(b == c)```의 결과가 참이면 두 번째 피연산자 하위 식이 평가될 것이며, 거짓이라면 세 번째 피연산자 하위 식이 평가될 것이다.  
```(b == c)```가 참이라고 가정하면 ```((d++) + (e * f))```가 평가될 것인데, 해당 식에는 다시 ```(d++)```와 ```(e * f)```라는 하위 식이 있다. 이 두 하위 식중에는 무엇이 먼저 평가될지 알 수 없다. 확실한 것은 두 하위 식이 먼저 평가된 후 ```결과값 + 결과값```이라는 식이 만들어지고, 이 더하기 연산자가 가장 마지막에 평가된다.  
```(b == c)```가 거짓이라고 가정하면 ```((g - (--h)) - i))```가 평가될 것이다. 이 식에서는 괄호 상 ```(--h)```가 가장 먼저 평가되고, 그 결과 값을 사용한 ```(g - 결과값)```이 다음으로 평가되며, 다시 그 결과 값을 사용한 ```결과 값 - i```이 마지막으로 평가되는 것이 보장된다.  
그러면 마지막으로 ```(b == c)```가 참이라면 두 번째 피연산자의 결과 값을 사용하고 거짓이라면 세 번째 피연산자의 결과 값을 사용하여 ```a = 결과값``` 이라는 식이 평가되는 것으로 마무리된다.  

하위 식의 평가 순서는 처음 보면 굉장히 혼란스럽다. 해당 내용이 어렵게 느껴진다면 굳이 모든 내용을 이해하려고 할 필요는 없다. 그저 여러 개의 연산자로 이루어진 복잡한 식이 있을 때, 연산자 우선 순위와 결합 방향을 통해서 완전 괄호 식을 구성할 수 있고, 완전 괄호 식을 만들었을 때 안쪽 괄호부터 평가하며, 같은 괄호 레벨에서 여러 개의 하위 식이 생긴다면 하위 식 간의 평가 순서는 알 수 없다 라고 이해하면 충분하다.  

그러면 ```j = i++ + i```와 같은 식을 다시 한번 생각해보자. 이를 완전 괄호 식으로 쓴다면 ```(j = ((i++) + (i)))```가 된다.  
```i++``` 의 결과 변수 ```i```에 저장된 값이 1이 증가하는건 사이드 이펙트이므로 값의 증가는 다음 시퀀스 포인트 이 전 아무때나 발생할 수 있다고 했다. 하지만 여기서는 증가 연산자가 평가되는 즉시 변수의 값이 증가한다고 가정해보자.  
그러면 위 식에서 ```(i++) + (i)```라는 하위 식이 있다. 여기서 다시 ```(i++)```와 ```(i)``` 라는 두 하위 식이 있다. 이 때 ```(i++)```가 먼저 평가된다면 ```i```의 값은 1이 증가된 값일 것이므로 ```j```에는 ```2 * i + 1```의 값이 대입될 것이다. 만약 ```(i)```가 먼저 평가된다면 ```i```의 값은 증가되지 않은 상태이므로 ```j```에는 ```2 * i```의 값이 대입될 것이다.  
이렇게 하위 식 간의 평가 순서는 보장되지 않으므로, 위에서 이야기한 논리 연산자/컴마 연산자/조건 연산자/함수 호출의 예외를 제외하고는 특정 하위 식이 먼저 평가될 것으로 가정한 코드를 작성하면 안 된다.  
사실 ```(a + b) - (a * b)```와 같은 식에서 ```a + b```가 먼저 평가되던 ```a * b```가 먼저 평가되던 결과 값에는 아무런 차이가 없다. 특정 오브젝트에 사이드 이펙트가 발생하는 경우만 차이가 생긴다. ```(i + 2) * (i = 3)```와 같은 식에서만 차이가 발생한다는 것이다. 그런데 애초에 하나의 오브젝트에 대해 사이드 이펙트가 발생한 이 후에 해당 오브젝트를 다음 시퀀스 포인트 이 전에 다시 사용하는 것은 미정의 행위이다. 따라서 이러한 코드는 더욱이 작성해서는 안 된다.  

여태까지 굉장히 복잡하고 어려운 연산자와 식의 평가 순서에 대해 알아보았다. 연산자 우선 순위와 결합 방향 표는 이 많은걸 외워야 하나 싶고, 하위 식 평가에 대한 내용은 난해해서 이걸 전부 이해해야 하나 싶다. 물론 이들을 모두 알고 있으면 좋겠지만 현실적으로 그러기 쉽지 않다. 여태까지 내용이 어려워서 이해하기 힘들었던 독자분들이라면 위 내용에 너무 집착하지 말고 아래에 저자가 제시하는 원칙만 기억하도록 하자. 사실 위의 내용은 아래 원칙들을 증명하기 위한 내용일 뿐이다.  

식 작성 원칙  
* 복잡한 식에서 연산자 우선 순위가 헷갈린다면 괄호를 사용하여 묶어주자
* 관계 연산자, 동등 연산자, 비트 연산자, 논리 연산자들의 조합으로 만들어진 복잡한 식은 꼭 의도에 맞도록 괄호를 사용하여 묶어주자
* 하나의 오브젝트에 대해 사이드 이펙트가 발생한 이 후 다음 시퀀스 포인트를 만나기 전까지는 해당 오브젝트를 다시 사용하지 말자
* 식은 최대한 이해하기 쉽도록 풀어서 작성하자

## 식 문장과 널 문장

C언어에는 여러 종류의 문장(statement)가 있다고 했다. 그 중 가장 대표적인 문장이 식 문장(expression statement)이다.  
C언어에서 모든 식은 ```;```으로 끝마치는 문장이 될 수 있다. 여태까지 우리는 ```a```, ```i++```, ```3 + 5```, ```"abc"```, ```a + b```, ```a = 3```, ```(a = ((b == c) ? ((d++) + (e * f)) : ((g - (--h)) - i)))```, ```printf("Hello, World!")```와 같이 다양한 식을 구성해 보았다. 이러한 식들의 끝에 구두점으로 ```;```을 쓰기만 해주면 하나의 문장이 되는 것이다.  
즉, ```a;```, ```i++;```, ```a + b;```, ```a = 3;```와 같은 것들은 모두 식 문장이다.  
여기서 한 가지 알아둬야 하는 것이 있다. 식 문장에 있는 식은 결국 void 식(void expression)으로 평가된다는 것이다.  

void 식이란 결과 타입이 ```void```인 식을 뜻한다. 여태까지 우리가 ```main``` 함수를 작성할 때 반환 타입을 ```int```로 작성하고, 인자 타입을 ```void```로 작성했다. 그리고 이제는 익숙해진 ```printf``` 함수도 사실 ```int``` 반환 타입을 가지며, 출력한 문자열의 바이트 수를 반환한다. 그런데 우리는 반환 타입이 ```void```인 함수를 작성할 수도 있다. 여태까지 우리가 작성한 ```main``` 함수는 ```return 0;```으로 0이라는 값을 반환하는데, 반환 타입이 ```void``` 함수는 이러한 반환 값이 없다. 이런 반환 타입이 ```void```인 함수를 호출하는 식이 대표적인 void 식이다. 참고로 void 식은 피연산자로 값을 필요로 하는 연산자의 피연산자로 사용할 수 없다.  
그렇다면 식 문장에 있는 식이 void 식(void expression)으로 평가된다는 것은 무엇을 의미할까? void 식에 대한 설명은 어려웠겠지만 결론은 굉장히 쉽다. __식의 최종 결과 값이 버려진다는 것이다.__ 예를 들어, ```3 + 5;``` 라는 식 문장의 식의 최종 결과 값은 8이다. 최종 결과 값 8은 식 문장이 끝나면서 버려지며 다음 문장에 영향을 주지 않는다. 마찬가지로 ```a = 3```의 결과 값은 3이다. 변수 ```a```에 사이드 이펙트로 3이 저장되겠지만, 결과 값 3은 버려져서 다음 문장에 영향을 주지 않는다. 결국 여러 개의 식 문장들이 있으면, 각각의 식 문장들은 이 전 식의 결과 값에 영향을 받지 않고 독립적으로 실행되는 것이다.  

```c
3 + 5;
a * b;      // 이 전 식 문장의 최종 결과 값 8이 a * b에 영향을 주지 않음
i = 3;      // 이 전 식 문장의 최종 결과 값 a * b가 i = 3에 영향을 주지 않음
i - 3;      // 이 전 식에서 발생한 사이드 이펙트로 i가 3이 되었지만, 결과 값 3은 i - 3에 영향을 주지 않음.
printf("abc");  // printf 함수는 출력한 문자열의 바이트 3을 반환함
r / 2;      // 이 전 식 문장의 최종 결과 값(printf 함수의 반환 값) 3이 r / 2에 영향을 주지 않음
```  

> 반환 타입이 ```int```인 함수 ```func```를 단독으로 호출하는 식 문장 ```func();```이 있을 때, 함수 ```func```의 반환 값이 버려지는 것도 같은 원리이다.  

식 문장 중에 한 가지 특이한 문장이 있다. 아무런 식을 쓰지 않은 빈 문장이다. 식을 쓰지 않고 단순히 ```;``` 만을 사용해서도 문장을 구성할 수 있다. 이렇게 식이 비어 있는 식 문장을 특별히 널 문장(null statement)라고 한다.  

```c
a = 3;      // a = 3이라는 식으로 만든 식 문장
3 + 5 * 8;  // 3 + 5 * 8이라는 식으로 만든 식 문장
;           // 식이 없는 널 문장
;;;         // 세 개의 널 문장
a + b;;     // a + b라는 식으로 만든 식 문장 뒤에 세미콜론이 하나 더 사용되어 널 문장이 이어붙음
```

널 문장은 아무런 동작을 하지 않는 문장이다. 컴파일 결과 널 문장은 어떤 기계어도 만들어내지 않는다.  
그렇다면 널 문장은 대체 왜 존재하는 것일까? 추후에 배울 선택문이나 반복문 챕터에서 일부 상황에 아무런 동작을 하지 않고 문장을 끝내야 하는 경우들이 있다. 이런 경우 아무런 동작 없이 문장을 종료할 수 있는 문법을 지원하기 위해 널 문장이 존재한다.  

## 상수 식

상수 식(constant expression)은 대입, 증가, 감소, 함수 호출, 컴마 연산자를 포함하지 않은 식으로 실행 시점이 아닌 컴파일 시점에 계산될 수 있는 식을 의미한다.  
예를 들어, 5 + 5와 같은 식은 정수 상수와 더하기 연산자로 이루어진 상수 식이다.  
상수 식은 컴파일 시점에 상수로 치환되기 때문에, 상수가 필요한 어느 문맥에서나 사용할 수 있다.  

상수 식에도 몇 가지 종류가 있지만, 지금 학습하기에는 버거운 내용들이 다수 포함된다.  
지금은 상수 식 중 정수 상수 식(integer constant expression)이 무엇인지만 알아보자.  

정수 상수 식은 다음과 같은 요소들로만 구성된 식이다.  
* 대입, 증가, 감소, 함수 호출, 컴마 연산자를 제외한 연산자
  * 캐스트 연산자는 정수형으로 캐스팅되는 경우만 허용
* 정수 상수
* 이름있는 상수
  * 열거형 상수
  * 미리 정의된 상수
  * constexpr 오브젝트
* 복합 리터럴 상수
  * constexpr 스토리지 클래스를 가지는 복합 리터럴
* 문자 상수
* 결과가 정수 상수인 sizeof 연산식
* alignof 식
* 정수로 즉시 캐스팅되는 실수 상수

아직 완전히 학습하지 않은 이름있는 상수나 복합 리터럴 상수 같은 것들을 제외하고 생각해보면 간단하다.  
예를 들어, ```10 + 5 - 2```라는 식은 정수 상수와 더하기/빼기 연산자만 사용하여 구성한 식이다. ```'a' + 1```도 문자 상수, 정수 상수, 더하기 연산자를 사용하여 구성한 식이고, ```(int)3.5 + 3```도 정수로 즉시 캐스팅되는 실수 상수와 정수 상수와 더하기 연산자를 사용하여 구성한 식이다. 이러한 식들은 정수 상수 식이다.  
반면, ```a + 3``` 이라는 식이 있을 때, ```a```가 이름 있는 상수가 아닌 변수인 경우 변수를 사용하였기 때문에 정수 상수 식이 될 수 없다. 또한, ```3 + f(1)```이라는 식에서는 함수 호출이 사용되었기 때문에 정수 상수 식이 될 수 없다.  

정수 상수 식은 주로 다음과 같은 곳에 사용되는데, 이를 지금 당장 이해할 필요는 없다. 우선 이런게 있다는 것만 알아두고, 추후에 각 내용을 학습하는 단원에서 정수 상수 식이 무엇이었는지 다시 떠올릴 수 있다면 충분하다.  
